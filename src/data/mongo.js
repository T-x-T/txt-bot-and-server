...
//Adds Text to all empty build_images fields in applications collection
if(config.data.db_upgrades.build_images_fill){
  main.get({build_images: ""}, "application", false, function(err, docs){
    docs.forEach((doc) => {
      doc.build_images = "automatically generated by build_images_fill database migration";
      main.edit(doc, "application", false, function(err, doc){
        if(err) console.log(err);
      });
    });
  });
