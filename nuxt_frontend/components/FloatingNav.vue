<template>

<div class="nav-container">
    <div @click="toggle()" class="mobile-nav mobileOnly">
      <transition name="fade" mode="out-in">
        <svg key=1 v-if="!isOpen" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="bars" class="svg-inline--fa fa-bars fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M16 132h416c8.837 0 16-7.163 16-16V76c0-8.837-7.163-16-16-16H16C7.163 60 0 67.163 0 76v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16zm0 160h416c8.837 0 16-7.163 16-16v-40c0-8.837-7.163-16-16-16H16c-8.837 0-16 7.163-16 16v40c0 8.837 7.163 16 16 16z"></path></svg>
        <svg key=2 v-if="isOpen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="3 0 20 20" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>
      </transition>
    </div>

<transition name="slide">
    <nav v-if="desktop || isOpen" @click="toggle()">


      <!--
          <div class="item" @click="scrollTo('section_features')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
            <h1 class="title">Features</h1>
          </div>

          <div class="item" @click="scrollTo('section_videos')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            <h1 class="title">Videos</h1>
          </div>
      -->
          <div class="item" @click="scrollTo('section_dynmaps')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            <h1 class="title">Worlds</h1>
          </div>
      <!--
          <div class="item" @click="scrollTo('section_stats')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 13v-1m4 1v-3m4 3V8M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>
            <h1 class="title">Paxterya in numbers</h1>
          </div>
      -->
          <div class="item" @click="scrollTo('section_slideshow')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            <h1 class="title">Screenshots</h1>
          </div>

          <div class="item" @click="scrollTo('section_rules')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>
            <h1 class="title">Rules</h1>
          </div>
      <!--
          <div class="item" @click="scrollTo('section_howToJoin')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
            <h1 class="title">How to join</h1>
          </div>
      -->
          <div class="item" @click="scrollTo('section_blog')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg>
            <h1 class="title">Blog</h1>
          </div>
          <div class="item" @click="scrollTo('section_downloads')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10" /></svg>
            <h1 class="title">Downloads</h1>
          </div>

          <div class="item" @click="scrollTo('section_memberList')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
            <h1 class="title">Members</h1>
          </div>

          <div class="item" @click="scrollTo('section_memberWorldmap')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
            <h1 class="title">World map</h1>
          </div>
      <!--
          <div class="item" @click="scrollTo('section_contactForm')">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            <h1 class="title">Contact us</h1>
          </div>
      -->
    </nav>
</transition>

</div>

</template>

<style lang="sass" scoped>
@import ~/assets/_vars.sass

.slide-enter-active,
.slide-leave-enter 
    transform: translateX(0)
    transition: all .3s linear

.slide-enter,
.slide-leave-to
    transform: translateX(100%)
 
.fade-enter-active, .fade-leave-active
    transition: .2s

.fade-enter, .fade-leave-to
    opacity: 0
    transform: scale(.5)


nav
  display: flex
  flex-direction: column
  position: fixed
  justify-content: center
  right: 0
  z-index: 200
  width: fit-content
  height: 100%
  border-right: 10px solid $pax-darkestcyan
  pointer-events: none

.item
  background: $pax-darkmodecyan1
  margin: 7px 0
  border-radius: 100px 0 0 100px
  display: flex
  height: 50px
  margin-right: -200px
  cursor: pointer
  pointer-events: initial
  &#apply
    background: $pax-yellow
    svg
      color: $pax-darkmodecyan2
    h1.title
      border-color: $pax-darkmodecyan2
      background: $pax-darkmodecyan1
      color: $pax-white
      @extend .pax-regular
  svg
    width: auto
    height: 55px
    color: $pax-lightcyan
    padding: 5px 7px 10px 12px
    box-sizing: border-box
    pointer-events: none
  h1.title
    font-size: 20pt
    padding: 4px 0 4px 10px
    height: 100%
    border-left: 10px solid $pax-darkcyan
    text-align: left
    text-transform: none
    box-sizing: border-box
    width: 200px
  &:hover
    margin-left: -180px

@media screen and ($mobile)
  .mobile-nav
    background: $pax-lightcyan
    color: white
    top: 0
    right: 0
    position: fixed
    width: 40px
    height: 40px
    padding: 5px 5px 20px 20px
    border-radius: 0 0 0 50px
    cursor: pointer
    z-index: 999
    border-left: 5px solid $pax-darkmodecyan2
    border-bottom: 5px solid $pax-darkmodecyan2

  nav
    pointer-events: initial
    background: rgba(0, 0, 0, .4)
    padding-left: 100vw
    overflow-y: auto
    .item
      margin: 7px 0
      &:hover
        margin: 7px 0
    


</style>

<script>
export default {

  data: () => ({
    isOpen: false,
    desktop: false
  }),

  mounted(){
    this.desktop = window.innerWidth >= 1000;
  },

  methods: {
    toggle: function(){
      this.isOpen = !this.isOpen
    },

    scrollTo(section){
      document.getElementById(section).scrollIntoView({behavior: "smooth", block: "start"});
      setTimeout(() => document.getElementById(section).scrollIntoView({behavior: "smooth", block: "start"}), 200);
    }
  }
}
</script>