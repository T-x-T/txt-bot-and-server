...
const main = {
  template: {
    ...
    overview(options, callback) {
      memberFactory.getAllWhitelisted()
        .then((members: any) => {
          ...
          main.template.mc({collection: 'overview'}, function (err, overview) {
            if(!err && overview) {
              let averageAge = 0;
    ...
    memberOverview(options, callback) {
      ...
      if(discord_id) {
        ...
        memberFactory.getByDiscordId(discord_id)
          .then((member: any) => {
            main.template.mc({uuid: member.getMcUuid(), collection: 'playtime'}, function (err, playtime) {
        ...
        memberFactory.getAllWhitelisted()
          .then((members: any) => {
            ...
            for(let i = 0; i < members.length; i++) {
              main.template.memberOverview({discord_id: members[i].getDiscordId()}, function (err, doc) {
                output.push(doc);
                if(err) error = true;
  ...
  templatePromise: {
    mc(options) {
      return new Promise((resolve, reject) => {
        main.template.mc(options, (err, res) => {
          if(err) {
            reject(err);
          } else {
            resolve(res);
