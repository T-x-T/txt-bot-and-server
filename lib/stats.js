...
stats.countryList = function(callback) {
  data.getMembers(false, false, true, function(docs) {
    if(docs) {
      ...
      countries = _internal.getCountries();
      ...
      let max = 0;
      for(key in countries) if(countries[key].numberOfThings > max) max = countries[key].numberOfThings;

      //Do the actual colorcoding
      for(key in countries) if(countries[key].numberOfThings > 0) countries[key].fillKey = ((countries[key].numberOfThings / max).toFixed(1) * 100) + '%';
...
_internal.getCountryIsoByName = function(object, value) {
  let iso = false;
  for(key in object) if(object[key].name === value) iso = object[key].ISO;
  return iso;
}
