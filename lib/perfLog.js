...
//Get all performance logs: Range with a value -> get all values newer than that date, if false get all
perfLog.getPerfLogs = function (range, callback) {
    if (range) {
        //Get entries from specific range
        perfLogModel.find({ timestamp: { $gte: Date.now() - range } }, function (err, docs) {
            if (err) {
                log.write(2, 'PerfLog: couldnt get values from range', { range: range }, function (err) { });
            } else {
                callback(docs);
            }
        });
        ...
        //Get all data
        perfLogModel.find(function (err, docs) {
            if (err) {
                log.write(2, 'PerfLog: couldnt get all values', { range: false }, function (err) { });
            } else {
                callback(docs);
            }
        });
