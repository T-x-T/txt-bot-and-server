                    ...
                    if(stats){
                      ...
                      data.getUserData(uuid, function(err, userData){
                        if(!err){
                          ...
                          log.write(1, 'mc_helpers.updateStats couldnt find the user object from the uuid', {err: err});
                        }
                      });
                    }
                  }else{
                    log.write(2, 'mc_helpers.updateStats couldnt read the stats from disk', {err: err});
                  }
                });
              }
            });
    ...
    if(rank){
        ...
        if(stats){
          ...
          data.getNewestMcStats(false, function(allStatsFull){
            if(allStatsFull){
              ...
              log.write(0, 'mc_helpers.getStatTemplate couldnt find the specified user', {user: discord});
      ...
      data.getNewestMcStats(discord, function(stats){
        if(stats){
          ...
          log.write(0, 'mc_helpers.getStatTemplate.general couldnt find the specified user', {user: discord});
          callback('I couldnt find any stats for the specified player', false);
        }
      });
    }
...
mc.rcon = function(cmd){
  ...
  rconCon.on('auth', function(){
    ...
    log.write(1, 'mc_helpers successfully authenticated to the rcon server', null);
    rconCon.send(cmd);
