...
mc.updateStats = function(){
  ...
  fs.readdir(path.join(__dirname, './../mc_stats/'), function(err, files){
    if(!err){
      ...
      files.forEach((file) => {
        ...
        fs.stat(path.join(__dirname, './../mc_stats/' + file), function(err, stats){
          if(!err){
            ...
            let uuid = file.replace('.json', '').replace('-','');
                ...
                fs.readFile(path.join(__dirname, './../mc_stats/' + file), 'utf8', function(err, fileData){
                  if(!err && fileData.length > 0){
                    ...
                    try{
                      ...
                      log.write(2, 'mc_helpers.updateStats couldnt save the new data', {err: e, data: fileData}, function(e){});
                    }
                    if(stats){
                      //Now we can find out which user these stats belong to
                      data.getUserData(uuid, function(err, userData){
                        if(!err){
                          data.addMcStats(userData.discord, stats, function(err){
                            if(err) log.write(2, 'mc_helpers.updateStats couldnt parse the data read from disk', {err: e, data: fileData}, function(e){});
                          });
                        }else{
                          log.write(1, 'mc_helpers.updateStats couldnt find the user object from the uuid', {err: err}, function(e){});
                        }
                      });
                    }
                  }else{
      ...
      log.write(2, 'mc_helpers.updateStats couldnt read the files from the directory', {err: err}, function(e){});
    }
  });
};
...
//Holder for all get stats templates functions; rank = true -> export the rank as well
mc.getStatTemplate = function(discord, collection, rank, callback){
        ...
        if(stats){
          ...
          data.getNewestMcStats(false, function(allStatsFull){
            if(allStatsFull){
              ...
              log.write(0, 'mc_helpers.getStatTemplate couldnt find the specified user', {user: discord}, function(e){});
