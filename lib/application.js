...
application.write = function(input, callback){
  ...
  mc_helpers.getUUID(input.mc_ign, function(uuid){
    if(uuid){
      //Get all applications of the member to find outif they already have accepted or pending review applications
      application.readAll({mc_uuid: uuid}, function(err, docs){
        let ok = true;
        docs.forEach((doc) => {
            ...
            if(input.discord_id && input.email_address && input.country && input.birth_month && input.birth_year && input.about_me && input.motivation){
              ...
              document.save(function(err, doc){
                if(!err){
                  ...
                  discord_helpers.sendMessage('New application from ' + input.mc_ign, config['new_application_announcement_channel'], function(err){});
                }else{
    ...
    if(doc){
      _internal.addNicks(doc, function(err, doc){
        if(!err){
          ...
          _internal.sendMail(doc.email_address, 'We have received your application', text);
        }else{
        ...
        applicationModel.findOneAndUpdate({id: id}, doc, function(err){
          if(!err){
            ...
            if(newStatus == 2){
              ...
              //Send the email
              _internal.sendMail(doc.email_address, 'Your application was unsuccessful :(', text);
              ...
              if(newStatus == 3){
                ...
                //Send the email
                _internal.sendMail(doc.email_address, 'Welcome! You got accepted :)', text);
          ...
          if(!err){
            ...
            discord_helpers.addMemberToRole(discord_id, discord_helpers.getRoleId('paxterya'), function(err){
              if(!err){
                ...
                discord_helpers.addIgnToNickById(app.discord_id);
...
_internal.sendMail = function(recipient, subject, text){
  ...
  mailTransporter.sendMail(mailOptions, function(err, info){
    if(err){
      log.write(2, 'application.sendMail couldnt sent the email out', {mail: mailOptions});
    }
  });
};
